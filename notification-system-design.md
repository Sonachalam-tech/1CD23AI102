...
```
score = typeWeight × (1 + recencyScore)
```

**Type weights (importance):**

| Type      | Weight | Reasoning                                              |
|-----------|--------|--------------------------------------------------------|
| Result    | 3      | Academic results directly affect a student's standing  |
| Event     | 2      | Time-sensitive but attendance is optional              |
| Placement | 1      | Important career opportunity but broader in nature     |

**Recency score:**

Recency is normalized between 0 and 1 across the full dataset:

```
recencyScore = (notifTimestamp - oldestTimestamp) / (newestTimestamp - oldestTimestamp)
```

A notification from right now gets recencyScore = 1. The oldest notification gets recencyScore approaching 0. Multiplying by `(1 + recencyScore)` means type weight dominates when recency is similar, but a much newer notification of a lower type can still outrank a stale higher-type notification.

---

#### How the Top N is Maintained Efficiently — Min-Heap

To find the top N notifications from a potentially large and continuously growing stream, a **min-heap of size N** is used.

**Why not just sort everything?**

Sorting all M notifications every time a new one arrives costs O(M log M). As the notification count grows to millions, this becomes unacceptably slow for a real-time inbox.

**Why a min-heap works better:**
